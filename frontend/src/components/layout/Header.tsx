...
const Header: React.FC = () => {
  ...
  return <header className={headerClasses}>
      <div className={`fixed top-0 left-0 w-full z-30 transition-all duration-300 ${isScrolled ? 'bg-gradient-to-r from-primary to-secondary shadow-lg' : 'bg-gradient-to-r from-primary to-secondary/80'}`}>
        <div className="container mx-auto flex flex-row items-center justify-between px-4 py-2 md:py-3">
          {/* Logo */}
          <Link to="/" className="flex items-center text-white font-accent text-2xl font-bold whitespace-nowrap">
            <span className="bg-black px-2 py-1 rounded-lg mr-2">Varanda da Conveniência</span>
          </Link>
          {/* Desktop Menu */}
          <nav className="hidden md:flex flex-row gap-4 items-center">
            {menuItems.map(item => (
              <button key={item.id} onClick={item.action ? item.action : undefined} className="text-white font-medium hover:text-accent transition-colors px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-accent">
                {item.label}
              </button>
            ))}
          </nav>
          ...
          <div className="flex flex-row items-center gap-2 md:gap-4">
            ...
            {/* Mobile Menu Button */}
            <motion.button className="md:hidden text-white hover:text-white/80 transition-colors ml-2" onClick={() => setIsMenuOpen(!isMenuOpen)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -50, opacity: 0 }} className="md:hidden bg-gradient-to-r from-primary to-secondary px-4 py-4 flex flex-col gap-2 shadow-lg">
              {menuItems.map(item => (
                <button key={item.id} onClick={item.action ? item.action : undefined} className="text-white font-medium text-lg py-2 px-2 rounded hover:bg-white/10 transition-colors text-left w-full">
                  {item.label}
                </button>
              ))}
              {/* User/Logout en menú móvil */}
              {isAuthenticated && (
                <button onClick={logout} className="text-white font-medium text-lg py-2 px-2 rounded hover:bg-white/10 transition-colors text-left w-full mt-2">Sair</button>
              )}
            </motion.div>
          )}
        </AnimatePresence>
