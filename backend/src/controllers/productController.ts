...
export const getAllProducts = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (req.query.sortBy) {
    ...
    sortQuery.createdAt = -1; // Por defecto, los más nuevos primero
  }
...
export const createProduct = asyncHandler(async (req: Request, res: Response) => {
  ...
  const uploadPromises = files.length > 0 ? files.map(async (file) => {
      // Optimizar imagen
...
export const uploadProductImages = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
